...
void TMXMap::addObjectLayer(TMXObjectLayer newLayer)
{
    m_objectlayerVector.push_back(newLayer);
}
...
TMXObjectLayer* TMXMap::getObjectLayer(std::string layerName)
{
    for (unsigned int index = 0; index < m_objectlayerVector.size(); ++index)
    {
        if (m_objectlayerVector[index].getName() == layerName)
        {
            return &m_objectlayerVector[index];
        }
    }
